...
SELECT
  json_build_object(
    ...
    (
      SELECT
        json_agg(
          json_build_object('id', m.id, 'poster_url', m.poster_url)
        )
      FROM
        movie m
    ),
    ...
    (
      SELECT
        json_agg(
          json_build_object('id', u.id, 'avatar_url', u.avatar_url)
        )
      FROM
        "user" u
    )
...
ALTER TABLE "movie"
RENAME COLUMN poster_url TO document_group_id;

ALTER TABLE "movie"
ALTER COLUMN document_group_id
TYPE INTEGER USING document_group_id::integer;

ALTER TABLE "movie"
ADD CONSTRAINT fk_document_group_id FOREIGN KEY ("document_group_id") REFERENCES "document_group" (id) ON DELETE SET NULL;
...
ALTER TABLE "user"
RENAME COLUMN avatar_url TO document_group_id;
...
----------------------------------------------------------------------------------------------------
CREATE
OR REPLACE FUNCTION delete_document_group () RETURNS TRIGGER AS $$
BEGIN
...
----------------------------------------------------------------------------------------------------
CREATE
OR REPLACE FUNCTION add_or_update_avatar (
  p_user_id INTEGER,
  p_data BYTEA,
