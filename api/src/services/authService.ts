...
class AuthService extends DatabaseService {
     ...
     */
    public async getUserByPseudoOrMail(pseudo: string, mail: string): Promise<{ pseudo: string; mail: string }> {
     ...
     */
    public async getPrivateUser(value: { id: number } | { pseudo: string } | { mail: string }): Promise<{
        id: number;
        pseudo: string;
        mail: string;
        role: ERoles;
     ...
     * @returns pgPromise
     */
    public async createUser(values: Record<'pseudo' | 'mail' | 'password', string>): Promise<void> {
