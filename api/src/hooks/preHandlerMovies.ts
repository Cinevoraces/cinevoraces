...
export default plugin((async (fastify, opts, done) => {
    ...
    fastify.decorate('throwIfMovieNotFound', async (request: Request<{ Params?: { id: number } }>, reply: Reply) => {
        ...
        if (!isMovie) _errorService.send(EErrorMessages.NOT_FOUND_MOVIE_ID, 404);
    });
    ...
    fastify.decorate(
        'throwIfMovieFound',
        async (request: Request<{ Body: { original_title: string; french_title: string } }>, reply: Reply) => {
            const { _errorService, _movieService } = fastify;
            ...
            if (isMovie) _errorService.send(EErrorMessages.ALREADY_POSTED_MOVIE, 401);
        },
    );
    ...
    fastify.decorate('throwIfProposalChecksFails', async (request: Request<{ Body: PostMovie }>, reply: Reply) => {
        ...
        if (proposedMovie) _errorService.send(EErrorMessages.USER_HAS_PROPOSAL, 401);

        // Movie existence check
        const movieExist = await _movieService.checkMovieExistanceByName(original_title, french_title);
        if (movieExist) _errorService.send(EErrorMessages.INVALID_PROPOSAL, 422);

        // Episode availability check
        const isEpisodeAvailable = await _movieService.checkEpisodeAvailability(episode_id);
        if (!isEpisodeAvailable) _errorService.send(EErrorMessages.UNAVAILABLE_EPISODE, 401);
    });
     ...
     */ // eslint-disable-next-line @typescript-eslint/no-unused-vars
    fastify.decorate(
        'adminThrowIfMovieIsPublished',
        async (request: Request<{ Params: { id: number } }>, reply: Reply) => {
            const { _errorService, _movieService } = fastify;
