import { EErrorMessages } from '@src/types';
...
export default plugin((async (fastify, opts, done) => {
    ...
    fastify.decorate('verifyPassword', async (request: Request, reply: Reply) => {
        ...
        if (!body || !(body as { password: string }).password) _errorService.send(EErrorMessages.MISSING_PASSWORD, 401);

        const isPasswordCorrect = await _authService.verifyPassword(userId, (body as { password: string }).password);
